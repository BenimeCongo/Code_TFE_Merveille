# -*- coding: utf-8 -*-
"""
Created on Wed Jan 28 20:02:15 2026
...
# PARAMÈTRES
# =========================================================
BASE_DIR     = r"D:\MEMOIRE Merveilles\DataSet\CICIDS_2019"
PARQUET_NAME = "CICIDS2019_train.parquet"
...
class QELM:
    ...
    def fit(self, X, Y_onehot):
        X = np.asarray(X, dtype=np.float32)
        self.W = self.rng.standard_normal((X.shape[1], self.n_hidden)).astype(np.float32) / np.sqrt(max(X.shape[1], 1))
...
# =========================================================
# Timestamp depuis Excel compatible
# =========================================================
...
def run_binary_models(X_train, X_test, y_train, y_test):
    ...
    print(report(pred))
    _store_result("binary", "QELM", met, extra={"TrainTime_s": float(t), **(rates or {}), **(aucs or {})})
    ...
    print(report(pred))
    _store_result("binary", "QNN", met, extra={"TrainTime_s": float(t), **(rates or {}), **(aucs or {})})
    ...
    print(report(pred))
    _store_result("binary", "QSVM", met, extra={"TrainTime_s": float(t), **(rates or {}), **(aucs or {})})
...
def save_report(console_text, results_dict, meta):
    ...
    out_dir = desktop / "RESULTATS_CICIDS2019_QUANTIQUE_INSPIRE_WORD"
    out_dir.mkdir(parents=True, exist_ok=True)

    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    base_name = out_dir / f"CICIDS2019_QINSPIRE_QELM_QNN_QSVM_{ts}"
    ...
    if DOCX_OK:
        doc = Document()
        doc.add_heading("CICIDS2019 — Rapport de résultats (QELM / QNN / QSVM)", level=1)
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("CICIDS2019 — Rapport de résultats (QELM / QNN / QSVM)\n")
...
try:
    ...
    #  ÉQUILIBRAGE UNIQUEMENT DU TRAIN MULTICLASSE
