# -*- coding: utf-8 -*-
"""
Created on Tue Jan 27 18:36:21 2026
...
# =========================================================
# (OPTIONNEL) — WORD EXPORT : python-docx import robuste
# =========================================================
DOCX_OK = True
try:
...
# =========================================================
# STOCKAGE STRUCTURÉ DES RÉSULTATS (pour interprétation Word)
# =========================================================
...
# =========================================================
# PARAMÈTRES  (ADAPTÉS À CICIDS_2017)
# =========================================================
...
# =========================================================
#   ÉQUILIBRAGE TRAIN MULTICLASSE À SUPPORT ÉGAL
# =========================================================
def balance_train_equal_support(df_train, label_col, time_col=None, target_k=None):
    vc = df_train[label_col].value_counts()
    if len(vc) == 0:
        return df_train.copy(), 0

    K = int(vc.min()) if target_k is None else int(target_k)
    K = max(1, K)

    parts = []
    for cls in vc.index:
        sub = df_train[df_train[label_col] == cls].copy()
        parts.append(sample_spread_in_class(sub, min(K, len(sub)), time_col=time_col))
...
# =========================================================
# RUN MODELS (avec stockage RESULTS)
# =========================================================
def run_binary_models(X_train, X_test, y_train, y_test):
    ...
    print(report(pred))
    _store_result("binary", "ELM", met, extra={"TrainTime_s": float(t), **(rates or {}), **(aucs or {})})
    ...
    print(report(pred))
    _store_result("binary", "NN", met, extra={"TrainTime_s": float(t), **(rates or {}), **(aucs or {})})

    # SVM
    h2("SVM Linéaire calibré — BINAIRE")
    ...
    print(report(pred))
    _store_result("binary", "SVM", met, extra={"TrainTime_s": float(t), **(rates or {}), **(aucs or {})})
...
def run_multiclass_models(X_train, X_test, y_train, y_test, class_names):
    ...
    # SVM
    h2("SVM Linéaire calibré — MULTICLASSE")
    svm = CalibratedClassifierCV(
        LinearSVC(random_state=RANDOM_STATE, max_iter=SVM_MAX_ITER),
        method="sigmoid"
    )
...
def _rank_and_interpret(rows, task_name):
    ...
    text.append("Pour des données IDS déséquilibrées, Macro-F1 et Balanced Accuracy sont prioritaires.")
    text.append(
...
def save_report(console_text, results_dict, meta):
    ...
    out_dir = desktop / "RESULTATS_CICIDS2017_CLASSIQUE_WORD"
    out_dir.mkdir(parents=True, exist_ok=True)

    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    base_name = out_dir / f"CICIDS2017_CLASSIQUE_ELM_NN_SVM_{ts}"
    ...
    if DOCX_OK:
        ...
        if meta.get("time_col", None) is not None:
            ...
            doc.add_paragraph("Time col : non disponible (ordre des lignes utilisé).")
...
# =========================================================
# MAIN (avec rapport auto en finally)
# =========================================================
report_path = None
time_col = None
...
try:
    ...
    show_counts(df_train_mc[LABEL_COL], "TRAIN multiclasse (avant équilibrage)", max_rows=50)
    show_counts(df_test_mc[LABEL_COL],  "TEST  multiclasse (naturel)", max_rows=50)

    #  ÉQUILIBRAGE UNIQUEMENT DU TRAIN MULTICLASSE (support égal)
    h1("MULTICLASSE — ÉQUILIBRAGE TRAIN À SUPPORT ÉGAL (TEST NATUREL)")

    df_train_mc_bal, K_train = balance_train_equal_support(df_train_mc, LABEL_COL, time_col=time_col, target_k=None)
    print(f"[INFO] Support cible K (TRAIN) = {K_train} par classe")
    show_counts(df_train_mc_bal[LABEL_COL], "TRAIN multiclasse (APRÈS équilibrage)", max_rows=50)
    show_counts(df_test_mc[LABEL_COL],      "TEST  multiclasse (reste naturel)", max_rows=50)

    # LabelEncoder: fit sur df_sample pour garder toutes les classes
    le = LabelEncoder().fit(df_sample[LABEL_COL])
    class_names = list(le.classes_)

    y_train_mc = le.transform(df_train_mc_bal[LABEL_COL])
    y_test_mc  = le.transform(df_test_mc[LABEL_COL])

    X_train_mc, X_test_mc = build_X(df_train_mc_bal, df_test_mc, time_col=time_col)
