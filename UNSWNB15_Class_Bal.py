# -*- coding: utf-8 -*-
"""
Created on Wed Jan 28 19:55:48 2026
...
# Colonnes candidates (UNSW peut avoir attack_cat pour multiclass + label pour binaire)
LABEL_COL_CANDIDATES = ["attack_cat", "Attack_cat", "Label", "label", "class", "Class"]

# Benign (UNSW : souvent "Normal" dans attack_cat, parfois 0 dans label)
...
# =========================================================
# OUTILS ROBUSTES (ANTI len(np.int64))
# =========================================================
def safe_len(x):
    """len() robuste: si x est un scalaire numpy -> renvoie 1 au lieu d'erreur."""
    try:
        return len(x)
    except Exception:
        return 1
...
def ensure_list(x):
    """Force x à devenir une liste Python (même si scalaire numpy)."""
...
def ensure_series(x):
    """Force x à être une pd.Series (même si scalaire)."""
...
def detect_time_col(df):
    # UNSW n'a pas toujours Timestamp. On tente quelques candidats.
...
def balance_train_equal_support(df_train, label_col, time_col=None, target_k=None):
    vc = pd.Series(df_train[label_col]).value_counts()
    vc = ensure_series(vc)
    if safe_len(vc) == 0:
    ...
    parts = []
    for cls in ensure_list(vc.index):
...
def save_report(console_text, results_dict, meta):
    ...
    out_dir = desktop / "RESULTATS_UNSW_NB15_CLASSIQUE_WORD"
    out_dir.mkdir(parents=True, exist_ok=True)

    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    base_name = out_dir / f"UNSW_NB15_CLASSIQUE_ELM_NN_SVM_{ts}"
...
try:
    ...
    # Benign: on tente "Normal", sinon si label binaire numeric {0,1} -> benign=0, sinon "BENIGN" si existe
