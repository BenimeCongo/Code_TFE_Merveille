...
# =========================================================
# (OPTIONNEL) — IMBLEARN (RandomUnderSampler)
# =========================================================
IMBLEARN_OK = True
try:
...
# =========================================================
# PARAMÈTRES (UNSW-NB15) — ESSENTIELS INCHANGÉS
# =========================================================
...
class QuantumInspiredFeatureMap(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.Wq = (rng.standard_normal((d, d)).astype(np.float32) / np.sqrt(max(d, 1)))
...
def build_majority_spread_sample(df, label_col, benign_label,
    ...
    for cls, supp in attack_supports.items():
        alloc[cls] = min(int(min_per_class), int(supp))
    ...
    if base > remaining:
        ...
        if left > 0:
            ...
            for cls in alloc:
                alloc[cls] = min(int(attack_supports.get(cls, 0)), int(alloc[cls] + int(extra.get(cls, 0))))
...
# =========================================================
# Split temporel par classe (70/30)
# =========================================================
...
# =========================================================
# ÉQUILIBRAGE TRAIN SEULEMENT (PAR LE BAS)
# =========================================================
def balance_train_only_down(X_train, y_train, mode="multiclass"):
    y = np.asarray(y_train)
    vc = pd.Series(y).value_counts()
    print(f"[BALANCE-DOWN] Avant ({mode}) : {vc.to_dict()}")

    if safe_len(vc) <= 1:
        print(f"[BALANCE-DOWN] Skip ({mode}) : 1 seule classe dans y_train.")
        return X_train, y_train

    min_count = int(vc.min())
    if min_count <= 0:
        print(f"[BALANCE-DOWN] Skip ({mode}) : min_count<=0.")
        return X_train, y_train

    # 1) RandomUnderSampler (si imblearn dispo)
    if IMBLEARN_OK:
        try:
            rus = RandomUnderSampler(random_state=RANDOM_STATE)
            Xb, yb = rus.fit_resample(X_train, y_train)
            vc2 = pd.Series(yb).value_counts()
            print(f"[BALANCE-DOWN] RUS OK ({mode}) : {vc2.to_dict()}")
            return Xb, yb
        except Exception as e:
            print(f"[BALANCE-DOWN] RUS échoué ({mode}) -> fallback maison. Raison: {e}")

    # 2) Fallback maison: sélectionner min_count par classe
    rng = np.random.default_rng(RANDOM_STATE)
    classes = ensure_list(vc.index)
    ...
    Xb = X_train[idx_keep]
    yb = np.asarray(y_train)[idx_keep]
    vc2 = pd.Series(yb).value_counts()
    print(f"[BALANCE-DOWN] Fallback maison OK ({mode}) : {vc2.to_dict()}")
    return Xb, yb
...
def run_binary_models(X_train, X_test, y_train, y_test):
    ...
    print(report(pred))
    _store_result("binary", "QELM", {**met, **(rates or {})}, extra={"TrainTime_s": float(t), **(aucs or {})})
    ...
    print(report(pred))
    _store_result("binary", "QNN", {**met, **(rates or {})}, extra={"TrainTime_s": float(t), **(aucs or {})})
    ...
    print(report(pred))
    _store_result("binary", "QSVM", {**met, **(rates or {})}, extra={"TrainTime_s": float(t), **(aucs or {})})
...
def _rank_and_interpret(rows, task_name):
    ...
    text.append(
        "Pour des données IDS potentiellement déséquilibrées, la Macro-F1 et la Balanced Accuracy "
        "sont plus robustes que l’Accuracy seule (car elles réduisent l’effet des classes majoritaires)."
    )
    text.append(
...
def save_report(console_text, results_dict, meta):
    ...
    out_dir = desktop / "RESULTATS_UNSWNB15_WORD"
    out_dir.mkdir(parents=True, exist_ok=True)

    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    base_name = out_dir / f"UNSWNB15_QELM_QNN_QSVM_BALDOWN_{ts}"
    ...
    if DOCX_OK:
        doc = Document()
        doc.add_heading("UNSW-NB15 — Rapport de résultats (QELM / QNN / QSVM)", level=1)
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("UNSW-NB15 — Rapport de résultats (QELM / QNN / QSVM)\n")
...
try:
    ...
    if BENIGN_LABEL not in uniq:
        # cas binaire 0/1
        try:
            if set(uniq).issubset({0, 1, np.int64(0), np.int64(1), "0", "1"}):
                BENIGN_LABEL = 0
            elif "BENIGN" in uniq:
                BENIGN_LABEL = "BENIGN"
        except Exception:
    ...
    h2("ÉQUILIBRAGE (TRAIN seulement) — MULTICLASSE (PAR LE BAS)")
    X_train_mc_bal, y_train_mc_bal = balance_train_only_down(X_train_mc, y_train_mc, mode="multiclass")

    run_multiclass_models(X_train_mc_bal, X_test_mc, y_train_mc_bal, y_test_mc, class_names)
    ...
    h2("ÉQUILIBRAGE (TRAIN seulement) — BINAIRE (PAR LE BAS)")
    X_train_b_bal, y_train_b_bal = balance_train_only_down(X_train_b, y_train_bin, mode="binary")

    run_binary_models(X_train_b_bal, X_test_b, y_train_b_bal, y_test_bin)
    ...
    try:
        meta = dict(
            ...
            time_col=_time_col_used
        )
        console_text = _console_buffer.getvalue()
        _report_path = save_report(console_text, RESULTS, meta)
