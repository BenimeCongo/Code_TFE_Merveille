# -*- coding: utf-8 -*-
"""
Created on Wed Jan 28 22:19:56 2026

@author: Merveille BAHOGWERHE
"""
...
# SVM: augmenter max_iter pour réduire les non-convergences
SVM_MAX_ITER = 5000
...
# =========================================================
RESULTS = {
    "multiclass": [],  # list of dicts
    "binary": []       # list of dicts
}
...
def try_add_timestamp_from_excel(df, base_dir, time_col_xls="Timestamp"):
    # 1) Si déjà présent dans le parquet (cas fréquent CICIDS2017)
    ...
    # 2) Sinon, tenter depuis un xlsx de même longueur
...
def run_binary_models(X_train, X_test, y_train, y_test):
    ...
    cm, rates = print_binary_rates(y_test, pred, "ELM BIN")
    aucs = auc_binary(y_test, proba, "ELM BIN")
    print(report(pred))
    _store_result("binary", "ELM", {**met, **rates}, extra={"TrainTime_s": float(t), **(aucs or {})})
    ...
    cm, rates = print_binary_rates(y_test, pred, "NN BIN")
    aucs = auc_binary(y_test, proba, "NN BIN")
    print(report(pred))
    _store_result("binary", "NN", {**met, **rates}, extra={"TrainTime_s": float(t), **(aucs or {})})
    ...
    cm, rates = print_binary_rates(y_test, pred, "SVM BIN")
    aucs = auc_binary(y_test, proba, "SVM BIN")
    print(report(pred))
    _store_result("binary", "SVM", {**met, **rates}, extra={"TrainTime_s": float(t), **(aucs or {})})
...
def _rank_and_interpret(rows, task_name):
    if not rows:
        return [], f"Aucun résultat enregistré pour la tâche {task_name}."

    df = pd.DataFrame(rows).copy()

    # Critère principal pour données déséquilibrées: Macro_F1 puis BAcc
    sort_cols = []
    if "Macro_F1" in df.columns:
        sort_cols.append("Macro_F1")
    if "BAcc" in df.columns:
        sort_cols.append("BAcc")
    if "Acc" in df.columns:
        sort_cols.append("Acc")
    ...
    text.append(
        "Les jeux de données IDS étant typiquement déséquilibrés, les indicateurs les plus "
        "représentatifs sont la Macro-F1 (performance moyenne par classe) et la Balanced Accuracy "
        "(sensibilité moyenne)."
    )
    ...
    if task_name == "binaire":
        # si ROC_AUC / PR_AUC existe
        if "ROC_AUC" in df_sorted.columns or "PR_AUC" in df_sorted.columns:
            b = best
            if "ROC_AUC" in b:
                text.append(f"ROC-AUC du meilleur modèle: {b['ROC_AUC']:.4f}.")
            if "PR_AUC" in b:
                text.append(f"PR-AUC(AP) du meilleur modèle: {b['PR_AUC']:.4f}.")
        # si TPR/FPR existe
        if "TPR" in best and "FPR" in best:
            text.append(
                f"En binaire, TPR={best['TPR']:.4f} (détection attaques) et FPR={best['FPR']:.4f} "
                "(fausses alertes). Un bon compromis recherche un TPR élevé et un FPR faible."
            )
    else:
        # multiclass: si ROC/PR macro/weighted dispo
        if "ROC_AUC_macro" in best or "PR_AUC_macro" in best:
            if "ROC_AUC_macro" in best:
                text.append(f"ROC-AUC OVR macro du meilleur modèle: {best['ROC_AUC_macro']:.4f}.")
            if "PR_AUC_macro" in best:
                text.append(f"PR-AUC macro du meilleur modèle: {best['PR_AUC_macro']:.4f}.")

    return df_sorted, "\n".join(text)

def save_report_word(console_text, results_dict, base_dir, meta):
    ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    out_dir = Path(base_dir) / "RESULTATS_CICIDS2017_WORD"
    out_dir.mkdir(parents=True, exist_ok=True)

    out_base = out_dir / f"CICIDS2017_ELM_NN_SVM_{ts}"

    # ----- Si python-docx dispo -> .docx
    if DOCX_OK:
        doc = Document()
        doc.add_heading("CICIDS2017 — Rapport de résultats (ELM / NN / SVM)", level=1)
        ...
        def add_table_from_df(title, df):
            doc.add_heading(title, level=3)
            if df is None or len(df) == 0:
                doc.add_paragraph("Aucun résultat.")
                return

            # Colonnes importantes (on évite tables gigantesques)
            preferred_cols = [
                "model", "TrainTime_s",
        ...
        add_table_from_df("2.1) Multiclasse", df_mc if isinstance(df_mc, pd.DataFrame) else pd.DataFrame())
        add_table_from_df("2.2) Binaire", df_bi if isinstance(df_bi, pd.DataFrame) else pd.DataFrame())
        ...
        out_path = str(out_base) + ".docx"
        doc.save(out_path)
        return out_path

    # ----- Sinon fallback TXT
    out_path = str(out_base) + ".txt"
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("CICIDS2017 — Rapport de résultats (ELM / NN / SVM)\n")
        f.write(f"Généré le : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write("=== Interprétation ===\n")
        _, txt_mc = _rank_and_interpret(results_dict.get("multiclass", []), "multiclasse")
        _, txt_bi = _rank_and_interpret(results_dict.get("binary", []), "binaire")
        f.write(txt_mc + "\n\n" + txt_bi + "\n\n")
...
# =========================================================
# MAIN (avec génération rapport Word en FIN)
# =========================================================
_report_path = None
meta = {}
...
try:
    ...
    # --- robustesse CICIDS2017: colonnes avec espaces
    ...
    # Timestamp si possible (silencieux)
    ...
    # Génération rapport (WORD/TXT) sur le Bureau
    try:
        ...
        _report_path = save_report_word(console_text, RESULTS, desktop, meta)

        print(f"[RAPPORT] Rapport généré sur le Bureau : {_report_path}")
        if not DOCX_OK:
